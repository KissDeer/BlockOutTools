...
1. `检查导入`：读取当前桥接 Actor，只生成新增、原地更新、不变、删除、保留与冲突计划，不修改关卡。
2. `导入 UE`：用户确认后执行已重新核对的增量计划，并通过 `set_editor_property` 的编辑器变更通知设置真实属性和触发构造更新。不要额外调用未由当前 UE Python 暴露的 `rerun_construction_scripts()`。

匹配顺序是 `LayoutToolsSync`稳定标记、原始`LayoutToolsId`、同类型唯一名称、完全一致几何。稳定标记由关卡名、模块实例或图层名、积木名称和类型确定，允许 AI 或重建流程改变临时 shape ID 后仍原地更新。名称只在双方都唯一时兜底；重复标记、重复名称或重复几何造成歧义时必须阻止 Apply，不能猜测后创建重叠 Actor。未匹配的旧 Actor 默认保留；只有勾选`删除网页中已不存在的桥接 Actor`并重新 dry-run 后才进入删除计划。

关卡启用白盒规范且`阻止不合格 UE 导入`开启时，规范错误同样阻止 Apply。规范警告不阻止。
...
- 增量计划中的新增、更新、删除、保留和冲突数量是否符合本次意图。
- 目标关卡、层高、坐标与旋转已检查。

桥接只处理 `BlockOutToolsBridge` 文件夹或标签拥有的 Actor，不自动保存 UE 关卡。单个新 Actor 在生成或配置中失败时必须立即销毁，不能在地图中遗留未标记的半成品；匹配 Actor 原地更新，不能通过先生成副本再删除旧对象的方式制造重叠。
