...
def validate():
    ...
    try:
        ...
        actors = [actor for actor in all_actors if actor.get_actor_label().startswith('COTD_')]
        ...
        for actor in actors:
            ...
            if isinstance(actor, unreal.StaticMeshActor):
                ...
                requires_collision = specs_by_label.get(label, {}).get('collision', True) is not False
                ...
                check(material is not None, 'material_assigned', actor=label, material=material_path)
    ...
    unreal.log('CATHEDRAL_VALIDATION_RESULT ' + json.dumps({key: report[key] for key in
