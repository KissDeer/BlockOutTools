"""Apply the reference-led Cathedral manifests in the target Unreal level.
...
ROOT = Path("D:/GameDesgin/BlockOutTools/output/cathedral-of-the-deep")
TARGET = "/Game/MyGame/Map/HighWallofLothric/L_HighWallofLothric_Test_GPT"
MATERIAL_ROOT = "/Game/MyGame/Map/HighWallofLothric/CathedralWhitebox/Materials/MI_COTD_"
SOURCES = ("v1-backup/plan.json", "architecture-v2.json", "grounds-v2.json")


def read_plan():
    documents = [json.loads((ROOT / name).read_text(encoding="utf-8")) for name in SOURCES]
    merged = copy.deepcopy(documents[0])
    merged["schema"] = "cathedral-whitebox-plan-v2"
    merged["refinement_sources"] = list(SOURCES)
    # Retain the original planning markers but remove floating text from the mood pass.
    for spec in merged["actors"]:
        if spec["kind"] == "text":
            spec.update(hidden=True, collision=False)
    ...
    merged["assumptions"].append(
        "V2 supersedes the original open-roof description: an architectural shell and reference-led exterior now enclose the nave."
    )
    for spec in merged["actors"]:
        if not spec["label"].startswith("COTD_") or spec["kind"] not in ("mesh", "text"):
...
def owned(actor):
    ...
    return "COTD_Whitebox" in tags or {"COTD", "CathedralWhitebox"}.issubset(tags)
...
def apply_actor(spec, actor, subsystem, current_level, origin, materials, meshes):
    ...
    if actor is None:
        ...
        actor.set_editor_property("tags", [unreal.Name("COTD"), unreal.Name("CathedralWhitebox"), unreal.Name("COTD_RefinementV2")])
    ...
    actor.set_folder_path(unreal.Name("COTD/" + spec["region"]))
...
def main():
    ...
    for index, spec in enumerate(plan["actors"], 1):
        ...
        if index % 100 == 0:
            print("COTD refinement applied %d / %d" % (index, len(plan["actors"])))
    ...
    write_json(ROOT / "refined-application.json", report)
    write_json(ROOT / "plan.json", plan)
    print("COTD_REFINEMENT " + json.dumps({key: value for key, value in report.items() if not key.endswith("actor_paths")}))
    print("REPORT " + str(ROOT / "refined-application.json"))
