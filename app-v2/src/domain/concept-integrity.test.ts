...
import { addLogicKey, addLogicLink, addLogicNode } from "./concept-commands";
...
describe("嵌套拓扑的导入完整性", () => {
  /*
   * 只留"这一层自己的身份与引用"这一类：模块分组（`scope.modules`）随模块层一起消失，
   * 它那几条坏数据（重复模块 / 模块区域缺失 / 区域重复分配）已经没有可坏的字段了。
   */
