...
import type { Block, BlockoutProfile, StairsLinearBlock, Vec3 } from "../../domain/types";

/** 楼板从**当前正在编辑的那份几何**里取：节点自己的几何或模块定义都行 */
export function StairLandingFields({ block, blocks, profile, onChange }: { block: StairsLinearBlock; blocks: Block[]; profile: BlockoutProfile; onChange: (block: StairsLinearBlock) => void }) {
  ...
  const surfaces = walkingSurfaces({ blocks });
