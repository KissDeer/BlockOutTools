...
function fitGeometry(geometry: EditableGeometry, width: number, height: number) {
  const model = createModulePreviewModel(geometry, width, height, 60);
 ...
 */
export function ModuleEditor({ geometry }: { geometry: EditableGeometry }) {
  ...
  // 草案是模块定义才有的旧能力；节点几何没有草案
  const draft = pendingDraft && geometry.moduleId && pendingDraft.projectId === project.projectId && pendingDraft.moduleId === geometry.moduleId ? pendingDraft : null;
  const previewing = Boolean(draft);
  const viewportKey = `${project.projectId}:${geometry.id}`;
  // 草案预览只换积木，其余字段（底图等）沿用当前几何
  const shownGeometry = useMemo(
    () => (draft && !showExisting ? { ...geometry, blocks: draft.blocks } : geometry),
    [geometry, draft, showExisting],
  );
  ...
  useEffect(() => {
    if (!draft || !geometry.moduleId || size.width <= 0 || size.height <= 0) return;
    if (previewViewport.current?.key !== viewportKey) previewViewport.current = { key: viewportKey, camera: cameraRef.current, width: size.width, height: size.height, baseline: geometryDigest(project, geometry.moduleId) };
    setShowExisting(false);
    setStageTransform(fitGeometry({ ...geometry, blocks: draft.blocks }, size.width, size.height));
    return;
    // A candidate change starts its own read-only camera. Canvas resizing is handled above.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [draft, viewportKey]);
  useEffect(() => {
    if (size.width <= 0 || size.height <= 0 || previewing) return;
  ...
  }, [previewing, viewportKey, stageTransform, size]);
  useEffect(() => {
  ...
  if (!shownGeometry) return <div className="empty-workspace">这份几何不存在或已被删除</div>;
  ...
  return (
    <div className="module-editor">
      <div className="editor-toolbar">
        ...
        {previewing && draft ? <button type="button" onClick={() => { const next = !showExisting; setShowExisting(next); setStageTransform(fitGeometry(next ? geometry : { ...geometry, blocks: draft.blocks }, size.width, size.height)); }}>{showExisting ? "查看草案" : "对比当前形态"}</button> : null}
        {geometry.reference && geometry.moduleId ? <ReferenceTools module={{ id: geometry.id, name: geometry.name, revision: 0, blocks: geometry.blocks, reference: geometry.reference }} /> : null}
      ...
      <div className={`konva-host ${spacePressed ? "is-panning" : ""}`} ref={containerRef}>
        ...
        >
          ...
          <Layer listening={false}>{geometry.reference ? <ReferenceUnderlay reference={geometry.reference} /> : null}</Layer>
