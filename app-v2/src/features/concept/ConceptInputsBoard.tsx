    ...
    <div className="inputs-board">
      ...
      <header className={`inputs-summary is-${completeness.ok ? "ok" : "blocked"}`}>
        ...
        <div className="inputs-summary-state">
          {completeness.ok
            ? <span className="state-ok">材料齐全，可以开始拆解</span>
            : <span className="state-blocked"><CircleAlert size={13} />缺少 {completeness.missing.map((kind) => INPUT_KINDS[kind].label).join("、")}，不能拆解</span>}
        </div>
      </header>
      ...
      <section className="inputs-proposal">
        <div className="proposal-row">
          <Stamp size={14} />
          {state === "none" ? <span>还没有登记拆解结果。</span> : null}
          {state === "current" ? <span className="state-ok">拆解结果与当前输入一致（基于 rev {proposal?.basedOnInputsRevision} · {proposal?.nodeCount} 区域 / {proposal?.linkCount} 链路）。</span> : null}
          {state === "stale" ? (
            <span className="state-blocked">
              <TriangleAlert size={13} />输入已变更，拆解结果需要重新核对（登记于 rev {proposal?.basedOnInputsRevision}，当前 rev {inputs.revision}）。
            </span>
          ) : null}
        </div>
        <div className="proposal-actions">
          <button
            ...
            title={completeness.ok ? "把当前逻辑拓扑登记为一次拆解结果" : "缺少必需输入"}
        ...
        {KIND_ORDER.map((kind) => {
          ...
          return (
            <section key={kind} className="inputs-group">
              ...
              {items.length === 0 ? (
                <p className="inputs-group-empty">{meta.required ? "还没有提供，拆解会被拒绝" : "还没有提供（可选）"}</p>
              ) : (
