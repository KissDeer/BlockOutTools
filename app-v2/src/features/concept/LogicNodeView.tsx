...
/** 节点内部几何的缩略图：与层级树、检查器共用同一套铺法，同一份几何在哪儿画都长一样 */
function InteriorPlan({ blocks, name }: { blocks: Block[]; name: string }) {
  return <NodePlanPreview blocks={blocks} name={name} width={PREVIEW_WIDTH} height={PREVIEW_HEIGHT} />;
}

export function LogicNodeView({ data, selected }: NodeProps) {
  const { node, interiorBlocks, childCount, linkCount, isStart, incoming, outgoing, showPreview } = data as LogicNodeData;
  const built = interiorBlocks.length > 0;
  const hasInterior = childCount > 0 || built;

  return (
    <div className={`logic-node role-${node.role} ${selected ? "is-selected" : ""} ${isStart ? "is-start" : ""} ${hasInterior ? "has-interior" : ""}`}>
      {[
      ...
      {showPreview && built ? (
        <div className="logic-node-preview">
          <InteriorPlan blocks={interiorBlocks} name={node.name} />
      ...
      {/* 里面还有什么：即使还没拼，也要先告诉人里面有几个东西、怎么连 */}
