...
export function AssemblyCanvas() {
  ...
  const edges = useMemo<Edge[]>(() => project.connections.map((connection) => ({
    ...
    markerEnd: ["stairs", "spiral-stairs", "elevator", "one-way-door", "locked-door", "shortcut", "one-way-elevator", "drop"].includes(connection.type) ? { type: MarkerType.ArrowClosed, color: "#d8a84e" } : undefined,
    style: {
      stroke: connection.id === selectedConnectionId ? "#eef2ee" : connection.type === "shortcut" ? "#6bd2b4" : connection.type.includes("door") ? "#4bb89a" : "#d8a84e",
      strokeWidth: connection.id === selectedConnectionId ? 3 : 2,
      strokeDasharray: connection.type.includes("elevator") || connection.type === "shortcut" ? "5 4" : connection.type === "drop" ? "2 5" : undefined,
    },
