...
import { scopesOf } from "../../domain/concept-scopes";
 ...
 * 项目资料的增删改与归属。
 *
 * 资料现在可以挂到**逻辑节点**上（`DesignMaterial.nodeIds`）：空数组 = 整个项目可见。
 * 模块层删除前这里挂的是模块，那时一份资料只有"属于某个模块"一种归属。
 */
export function MaterialManager() {
  ...
  /** 点"选择图片"那一刻要改的是哪份资料；null = 新建 */
  const editingIdAtPick = useRef<string | null>(null);
  const [reading, setReading] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);
  const identity = useRef(project.projectId);
  identity.current = project.projectId;

  /** 资料能挂到的去处：拓扑里的全部节点（含子层） */
  const nodes = useMemo(
    () => (project.concept ? scopesOf(project.concept).flatMap((scope) => scope.nodes) : []),
    [project.concept],
  );
  ...
  async function readFile(file: File) {
    ...
    const editingId = editingIdAtPick.current;
  ...
  function saveMaterial() {
    ...
    // 新建时先挂整个项目；要限定到某个区域，用下面那份资料行上的勾选框
    const material: DesignMaterial = { id: createId("material"), name: name.trim(), kind, text, imageData, nodeIds: [] };
  ...
  function assignMaterial(id: string, nextIds: string[]) {
    ...
    state.acceptProject({ ...state.project, designContext: { ...context, materials: context.materials.map((item) => item.id === id ? { ...item, nodeIds: nextIds } : item) }, updatedAt: new Date().toISOString() });
  }
  ...
  function removeMaterial(id: string) {
    ...
    if (!material) return;
    // 原图只存一份，删除会让所有关联区域一起失去它，所以说清影响面
    const affected = material.nodeIds.length;
    ...
    setStatus(affected
      ? `已删除“${material.name}”，${affected} 个关联区域不再引用它；可用撤销恢复`
  ...
  return <div className="material-manager">
    ...
    <button type="button" className="context-action" onClick={() => setAdding((value) => !value)} aria-expanded={adding}><Plus size={13} />添加资料</button>
    {adding ? <div className="context-material-form">
      ...
      <div className="context-actions"><button type="button" onClick={() => { editingIdAtPick.current = null; fileRef.current?.click(); }} disabled={reading}><ImagePlus size={13} />{imageData ? "更换图片" : "选择图片"}</button><button type="button" disabled={reading || !name.trim() || (!text.trim() && !imageData)} onClick={saveMaterial}>保存资料</button></div>
    ...
    {materials.length > 0 ? <details className="context-material-sharing"><summary>关联已有资料 · {materials.length} 份{duplicateGroups.size ? ` · ${duplicateGroups.size} 组同名` : ""}</summary>
      <p className="context-muted">同一份资料可关联多个区域，原图仅保存一份。</p>
      ...
      {materials.map((material) => <div className={`material-sharing-row${variantOf(material.id) ? " is-duplicate" : ""}`} key={material.id}>
        ...
        <details className="context-material-form"><summary>编辑资料 · 所有关联区域同步</summary><label>名称<input key={`name:${material.name}`} defaultValue={material.name} onBlur={(event) => editMaterial(material.id, "name", event.target.value)} /></label><label>说明<textarea key={`text:${material.text}`} rows={2} defaultValue={material.text} onBlur={(event) => editMaterial(material.id, "text", event.target.value)} /></label><button type="button" disabled={reading} onClick={() => { editingIdAtPick.current = material.id; fileRef.current?.click(); }}>{material.imageData ? "更换原图" : "补充原图"}</button></details>
        ...
        <label className="context-checkbox"><input type="checkbox" checked={material.nodeIds.length === 0} onChange={(event) => assignMaterial(material.id, event.target.checked ? [] : nodes.slice(0, 1).map((node) => node.id))} disabled={nodes.length === 0} />整个项目</label>
        {nodes.map((node) => <label key={node.id} className="context-checkbox"><input type="checkbox" checked={material.nodeIds.includes(node.id)} onChange={(event) => {
          const next = event.target.checked ? [...material.nodeIds, node.id] : material.nodeIds.filter((id) => id !== node.id);
          if (!next.length) { setStatus("至少关联一个区域；若需全项目共用，请选择“整个项目”"); return; }
          assignMaterial(material.id, next);
        }} />{node.name}</label>)}
        {material.nodeIds.length ? <p className="context-muted">已限定到：{material.nodeIds.map(nodeName).join("、")}</p> : null}
      </div>)}
      {nodes.length === 0 ? <p className="field-help">还没有逻辑区域，资料暂时只能挂在整个项目上。</p> : null}
    </details> : null}
