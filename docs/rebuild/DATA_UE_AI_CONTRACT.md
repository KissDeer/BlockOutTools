...
解析后的资料（`ResolvedDesignMaterial`，仅存在于上下文与请求，不落盘）在持久化字段之外补三项派生信息：

```ts
{ contentDigest: string; variantLabel: string; origin: "project-material" | "module-material" | "scope-input" | "module-reference" }
```

- `contentDigest`：有原图取原图指纹，否则取文字指纹。
- `variantLabel`：同名资料的序号（`同名 1/2`），不重名时为空串。
- `origin`：资料真正存放的位置，用于把"去哪里改/删"指对入口（`scope-input` → 历史资料管理，其余 → 模块参考栏的资料管理）。

**同名资料不自动取舍**：内容完全相同时提示"保留任意一份即可"；同名但内容不同时提示无法判断当前版本、本次全部保留并给出编号，等待人工确认。不按名称猜配、不静默丢弃任何一份。
...
**采用前显示规范检查影响**：预览期间用 `previewModuleDraft` 试算套用后的项目（不写入），对比当前与试算结果的 `validateProject` 差异，列出"会新增/会消除/原本就有"的问题。试算结果不是合法项目时明确说明"采用会被拦下"，不显示假结论。规范检查未启用时不制造结论。
